...
def build_payload(pub_cards: List[str], pot: int, ante: int, btn_username: str, bb_username: str, sb_username: str, infos: List[str]) -> List[object]:
    ret = [{
        "type": "divider"
    }]
    ...
    ret.append({
        ...
        "text": {
            "type": "mrkdwn",
            "text": f"*total pot: ${pot}\t\tlevel: ${ante // 2}/${ante}\t\tbtn: {btn_username}\t\tbb: {bb_username}\t\tsb: {sb_username}*"
        }
    })
